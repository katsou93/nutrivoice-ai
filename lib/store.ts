...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      profile: null,
      plan: null,
      setProfile: (p) => set({ profile: p, plan: calculatePlan(p) }),
      logs: {},
      addEntry: (date, entry) =>
        set((s) => ({ logs: { ...s.logs, [date]: { ...s.logs[date], date, entries: [...(s.logs[date]?.entries ?? []), entry] } } })),
      removeEntry: (date, id) =>
        set((s) => ({ logs: { ...s.logs, [date]: { ...s.logs[date], entries: (s.logs[date]?.entries ?? []).filter((e) => e.id !== id) } } })),
      logWeight: (date, kg) =>
        set((s) => ({ logs: { ...s.logs, [date]: { ...s.logs[date], date, entries: s.logs[date]?.entries ?? [], weight: kg } } })),
      logWater: (date, glasses) =>
        set((s) => ({ logs: { ...s.logs, [date]: { ...s.logs[date], date, entries: s.logs[date]?.entries ?? [], water: glasses } } })),
      today: todayStr,
      todayLog: () => { const d = todayStr(); return get().logs[d] ?? { date: d, entries: [] } },
      activities: {},
      addActivity: (date, activity) =>
        set((s) => ({ activities: { ...s.activities, [date]: [...(s.activities[date] ?? []), activity] } })),
      removeActivity: (date, id) =>
        set((s) => ({ activities: { ...s.activities, [date]: (s.activities[date] ?? []).filter(a => a.id !== id) } })),
      getTotalBurnedToday: () => (get().activities[todayStr()] ?? []).reduce((sum, a) => sum + a.caloriesBurned, 0),
      addPlannedMeal: (date, meal) =>
        set((s) => ({ logs: { ...s.logs, [date]: { ...s.logs[date], date, entries: s.logs[date]?.entries ?? [], plannedMeals: [...(s.logs[date]?.plannedMeals ?? []), meal] } } })),
      removePlannedMeal: (date, id) =>
        set((s) => ({ logs: { ...s.logs, [date]: { ...s.logs[date], plannedMeals: (s.logs[date]?.plannedMeals ?? []).filter(m => m.id !== id) } } })),
      setPlannedNote: (date, note) =>
        set((s) => ({ logs: { ...s.logs, [date]: { ...s.logs[date], date, entries: s.logs[date]?.entries ?? [], plannedNote: note } } })),
      confirmPlannedMeal: (date, id) =>
        set((s) => {
          const planned = s.logs[date]?.plannedMeals?.find(m => m.id === id)
          if (!planned) return s
          const entry: FoodEntry = { id: crypto.randomUUID(), name: planned.name, calories: planned.calories, protein: planned.protein, carbs: planned.carbs, fat: planned.fat, amount: planned.amount, unit: planned.unit, mealType: planned.mealType, timestamp: new Date().toISOString(), confidence: 'estimated' }
          return { logs: { ...s.logs, [date]: { ...s.logs[date], entries: [...(s.logs[date]?.entries ?? []), entry], plannedMeals: (s.logs[date]?.plannedMeals ?? []).filter(m => m.id !== id) } } }
        }),
      points: 100,
      ...
      unequipItem: (category) => set(s => { const eq = { ...s.equippedItems }; delete eq[category]; return { equippedItems: eq } }),
      streak: 0,
      lastLogDate: '',
      updateStreak: (date) => set(s => {
        const yesterday = new Date(Date.now() - 86400000).toISOString().split('T')[0]
        if (s.lastLogDate === date) return s
        return { streak: s.lastLogDate === yesterday ? s.streak + 1 : 1, lastLogDate: date }
      }),
      healthProfile: '',
